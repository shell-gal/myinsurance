...
function load(){
	$.ajax({
			...
			success:function(data){
				$(".employeesTel").val(data.singerData.tel);
...
function upd(){
	$("#upd").click(function(){
		$.ajax({
			url:"http://localhost:8080/users/updMyInfo.do",
			type:"get",
			data:{
				"tel":$(".employeesTel").val(),
			...
			success:function(data){
				alert(data.message);
				location.href="http://localhost:8080/src/page/settle_lh/message.jsp";
			},
			error:function(){
				alert("System error!!!!!");
			}
